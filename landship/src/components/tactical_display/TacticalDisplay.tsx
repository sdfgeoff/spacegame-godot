...
export const TacticalDisplay = ({
  ...
  const [orbitRef, setOrbitRef] = React.useState<any /*OrbitControls3*/ | null>(
    null,
  );
  ...
  const onCameraMove = React.useCallback(() => {
    ...
    const visibleRadius =
      dist * Math.sin(THREE.MathUtils.degToRad(camera.fov / 2));
  ...
  return (
    <Canvas
      {...props}
      camera={{ position: [0, 0, -45], fov: 30, near: 1.0, far: 1000.0 }}
    >
