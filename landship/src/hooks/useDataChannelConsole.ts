...
import {
  DataChannelConsole,
  DataChannelState,
} from "../network/DataChannelConsole";
...
type SubscribeToTopic = <T extends Topic>(
  topic: T,
...
export const useDataChannelConsoleInternal = (
  ...
  const [connectionState, setConnectionState] =
    useState<DataChannelState>("connecting");
  ...
  const subscribeTopic = useCallback(
    <T extends Topic>(
      ...
      setTopicSubscriptions((oldSubscriptions) => [
        ...oldSubscriptions,
        { uid, topic },
      ]);
      ...
      return () => {
        unsub();
        setTopicSubscriptions((oldSubscriptions) =>
          oldSubscriptions.filter((s) => s.uid !== uid),
        );
      };
    },
    [dataChannelConsole],
