...
export const usePingShip = (
  dataChannelConsole: DataChannelConsole | undefined,
) => {
...
export const useTimingStatsInternal = (
  dataChannelConsole: DataChannelConsoleInternal,
): TimingStats | undefined => {
  const [timingStats, setTimingStats] = useState<TimingStats | undefined>(
    undefined,
  );
    ...
    return dataChannelConsole.subscribeTopic("Pong", (message) => {
      ...
      setTimingStats((oldStats) => {
        ...
        const newClockOffset = oldStats?.clockOffset
          ? oldStats.clockOffset * (1 - CLOCK_OFFSET_DAMPING) +
            clockOffset * CLOCK_OFFSET_DAMPING
          : clockOffset;
...
export const useServerTime = () => {
  ...
  return {
    ...
    currentServerTime: timingStats?.clockOffset
      ? (performance.now() - timingStats.clockOffset) / 1000
      : undefined,
  };
};
